...
function display_return(msg) {
	function explines() {
		...
		spn.setAttribute('style', 'position:relative;width:100%;height:100%;');
		...
		spn1.setAttribute('style', 'position:absolute;top:0px; left:' + (pos - 1) + '%;width:320px;border:0px;padding:15px;border-radius:5px;font-size:16px;color:#eeeeee;background-color:green;');
		var textNod = msg;
		spn1.innerHTML = textNod;
		spn.appendChild(spn1);
		var incopc = 0;
		document.getElementById('bk').appendChild(spn);
		function cgnopc_inv() {
...
form.addEventListener('submit', function (ev) {
	...
	stripe.confirmCardPayment(form.dataset.secret, {
		payment_method: {
			card: numElm
			//return_url: 'https://localhost/check-one-time-php/public/success.php'
		}
		...
		if (result.error) {
			...
			if (result.paymentIntent.status === 'succeeded') {
				...
				console.log(result.paymentIntent.list);
				async function store_id() {
					let response = await fetch('updatedb.php', {
						method: 'POST',
						headers: {
						...
						body: JSON.stringify({
							client_id: document.querySelector('#payment-form').id.value,
				...
				//sts.innerHTML = `              
				//         <p style='font-size: 14px;'><b>Montante deduzido: </b>R$ ${result.paymentIntent.amount/100} ,00</p>
				//  `
